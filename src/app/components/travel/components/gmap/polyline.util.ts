...
export class PolylineUtil {
  ...
  createPolylines(journeys: Array<Array<Point>>, map: google.maps.Map, strokeColor: string): Array<google.maps.Polyline> {
    const lines: Array<google.maps.Polyline> = []
    journeys.forEach((_journey: Array<Point>, index: number) => {
  ...
  createDottedPolylines(journeys: Array<Array<Point>>, map: google.maps.Map, strokeColor: string): Array<google.maps.Polyline> {
    const lines: Array<google.maps.Polyline> = []
    journeys.forEach((_journey: Array<Point>, index: number) => {
  ...
  pushToPolylines(polylines: Array<google.maps.Polyline>, journeys: Array<Array<Point>>): void {
    journeys.forEach((journey: Array<Point>, index: number) => {
      const polyline: google.maps.Polyline = polylines[index]
      this._lineDrawWait++
      journey.forEach((leg: Point) =>
        delay(this._lineDrawWait * 130)
          .then(() => {
